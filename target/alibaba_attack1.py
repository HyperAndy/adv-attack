...
# epsilon = random.uniform((FLAGS.max_epsilon - 10), FLAGS.max_epsilon)
...
def input_diversity(input_tensor):
    ...
    # input_tensor = tf.image.rot90(input_tensor, k=rand)
...
def graph(x, y, i, x_max, x_min, grad):
    ...
    ########################
    # logits = (end_points_inc_v1['Logits'] + end_points_res_v1_50['logits'] + end_points_vgg_16['logits']) / tf.pow(tf.constant(1.5, dtype=tf.float32), tf.multiply(tf.cast(tf.subtract(tf.constant(FLAGS.num_iter), i), dtype=tf.float32), tf.constant(0.1, dtype=tf.float32)))
    ...
    # mean_square_error = tf.losses.mean_squared_error(one_hot, softmax)
    # # variance reduce
    # x_out = tf.constant(np.zeros(x.shape), tf.float32)
    # step = tf.constant(0)
    # def loop(x, step, x_out):
    #     x_out = tf.add(x_out, tf.random_normal(x.shape, mean=x, stddev=sigma))
    #     step = tf.add(step, 1)
    #     return x, step, x_out
    #
    ...
    #
    # noise = tf.gradients(cross_entropy, x_out)[0]
    ...
    noise = FLAGS.momentum * grad + 0.99 * noise
    noise = noise / tf.reshape(
        tf.contrib.keras.backend.std(tf.reshape(noise, [FLAGS.batch_size, -1]), axis=1),
        [FLAGS.batch_size, 1, 1, 1])
    # noise = noise / tf.sqrt(tf.reduce_mean(tf.square(noise), [1, 2, 3], keep_dims=True))  # 使用l2范数
