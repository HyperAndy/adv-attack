...
def jrelu(x):
    if tf.greater(x, tf.constant(delta)) is not None: return x
    else: return 0.0

def jrelu_grad(x):
    if tf.greater(x, tf.constant(delta)) is not None: return 1.0
    else: return 0.0
...
jrelu_np_32 = lambda x: jrelu(x).astype(x.dtype)
jrelu_grad_np_32 = lambda x: jrelu_grad(x).astype(x.dtype)
